...
const currentStepComponent = computed(
  () => steps[formStore.currentStep].component
);
...
//   data: Partial<FormData> & { isNextEnabled: boolean }
// ) => {
//   // formStore.updateFormData(data);
//   // formStore.setNextEnabled(data.isNextEnabled);
// };
...
watch(
  ...
  () => {
    if (
      ...
      authStore.errors.middlename ||
      authStore.errors.lastname
    ) {
      goToStep(0);
    } else if (authStore.errors.line1 || authStore.errors.barangay) {
      goToStep(1);
    } else if (authStore.errors.email || authStore.errors.password) {
      goToStep(2);
    }
  },
  { deep: true }
...
<template>
  ...
  <div
    class="p-6 flex flex-col sm:flex-row justify-between space-y-2 sm:space-y-0 mt-4 w-full"
  >
    <button
      class="!border-white text-white text-lg p-3 rounded transition w-full sm:w-auto"
      @click="prevStep"
    ...
    <button
      ...
      @click="nextStep"
      :disabled="!formStore.isNextEnabled"
    >
      Next
    </button>
    <button
    ...
    <a
      class="border-2 border-white text-white text-lg text-center p-3 rounded transition w-full sm:w-auto"
      href="/"
      >Cancel</a
    >
  </div>
